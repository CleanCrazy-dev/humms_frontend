...
export function useAllTokens(): { [address: string]: Token } {
  ...
  const chain = chainId ?? 1088;

  const allTokens = flatten(Object.values(TOKENS)).reduce<{ [address: string]: Token }>(
    (prev, curr) => {
      prev[curr.address[chain]] = new Token(chain, curr.address[chain], curr.decimals, curr.symbol, curr.name);
  ...
  return useMemo(() => {
    ...
    if (decimals.result) {
      return new Token(
        ...
        address,
        decimals.result[0],
        parseStringOrBytes32(symbol.result?.[0], '', 'UNKNOWN'),
        // parseStringOrBytes32(symbol.result?.[0], symbolBytes32.result?.[0], 'UNKNOWN'),
        parseStringOrBytes32(tokenName.result?.[0], '', 'Unknown Token')
...
export function useCurrency(currencyId: string | null | undefined): Currency | null | undefined {
  ...
  if (currencyId === null || currencyId === undefined) return currencyId as null | undefined;
  return token;
}