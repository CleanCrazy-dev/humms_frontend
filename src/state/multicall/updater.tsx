...
import { useMulticall2Contract } from '@hooks/contracts/useMulticallContract';
...
import type { Call, MulticallState } from './types';
...
async function fetchChunk(
  ...
  try {
    ...
    if (isDebug) {
      (returnData as any[]).forEach(({ gasUsed, returnData, success }, i) => {
        if (
          !success &&
          returnData.length === 2 &&
